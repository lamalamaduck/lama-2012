...
%
    \begin{centerautomata}
      ...
      \path
      (q0) edge              node        {$a \to \_,R$}  (qa)
      (q0) edge              node        {$\_ \to R$}    (a)
      (q0) edge              node [swap] {$b \to \_,R$}  (qb)
      (qa) edge [loop above] node        {$a \to a, R$}  (qa)
      (qa) edge [bend left]  node        {$b \to a, R$}  (qb)
      (qa) edge [bend right] node [swap] {$\_ \to a, R$} (a)
      (qb) edge [bend left]  node        {$a \to b, R$}  (qa)
      (qb) edge [loop below] node        {$b \to b, R$}  (qb)
      (qb) edge [bend left]  node        {$\_ \to b, R$} (a);
    \end{centerautomata}
  \item
